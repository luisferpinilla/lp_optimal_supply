...
class Simulation:
    def __init__(self, order_cutoff:float, order_target:float, lead_time:float):
    ...
    def handle_customer_event(self):
        ...
        if demand < self.inventory:
            print(f'se han vendido {demand} unidades')
            # actualizar revenue
        ...
        if self.inventory < self.order_cutoff and self.number_orderer == 0:
            ...
            print(f'se ha colocado una orden por {self.number_orderer} unidades que llegarán en {self.t_delivery}, lo que deja el costo de ordenar en {self.cost_orders}')
...
s = Simulation(order_cutoff=10, order_target=30)
